    ...
    if (updates.status) {
      ...
      if (updates.status !== oldStatus) {
        ...
        if (updatedBooking && trip) {
          ...
          const organizerProfile = await userService.getUserById(
            trip.createdBy
          );
          const organizerName =
            trip.host?.name || organizerProfile?.name || "Organizer";
          ...
          if (updates.status === "Approved") {
            ...
            message =
              `Great news! Your booking request for ${trip.title} has been approved by ${organizerName}.\n\n` +
              `Trip Details:\n` +
              `- Location: ${trip.about?.location || "Various locations"}\n` +
              `- Dates: ${trip.about?.startDate || "TBD"} to ${
                trip.about?.endDate || "TBD"
              }\n` +
              `- Group Size: ${updatedBooking.groupSize} ${
                updatedBooking.groupSize === 1 ? "person" : "people"
              }\n` +
              `- Total Amount: ₹${updatedBooking.totalAmount.toLocaleString(
                "en-IN"
              )}\n\n` +
            ...
            message =
              `We regret to inform you that your booking request for ${trip.title} has not been approved at this time.\n\n` +
              `Trip Details:\n` +
              `- Location: ${trip.about?.location || "Various locations"}\n` +
              `- Dates: ${trip.about?.startDate || "TBD"} to ${
                trip.about?.endDate || "TBD"
            ...
            message =
              `Your booking for ${trip.title} status has been updated to Pending.\n\n` +
              `Trip Details:\n` +
              `- Location: ${trip.about?.location || "Various locations"}\n` +
              `- Dates: ${trip.about?.startDate || "TBD"} to ${
                trip.about?.endDate || "TBD"
